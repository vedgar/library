...
assert wordlist == sorted(wordset, key=lambda w: (len(w) == 4, w))

def w2n(word):
    r = 0
    for letter in word:
        r = r * len(alphabet) + alphabet.index(letter)
    return r
...
def w2nw(word):
    return w2n(word), word

nums = sorted(map(w2nw, wordlist))
difs = [(n2 - n1, w1, w2) for (n1, w1), (n2, w2) in zip(nums, nums[1:])]
print(sorted(difs)[-10:])
